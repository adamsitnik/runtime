...
{
    public class AcceptMultishot
    {
        public static IEnumerable<object[]> AddressFamilies()
        {
        ...
        private static Socket CreateListener(AddressFamily family = AddressFamily.InterNetwork)
        {
            Socket listener = new(family, SocketType.Stream, ProtocolType.Tcp);
        ...
        [Theory]
        [MemberData(nameof(AddressFamilies))]
        public async Task Burst_PreservesOrderAndResumes(AddressFamily family)
        {
            using Socket listener = CreateListener(family);
            await using IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> first = enumerator.MoveNextAsync().AsTask();
            Socket[] clients = new Socket[256];
            try
            {
                for (int i = 0; i < clients.Length; i++)
                {
                    clients[i] = new Socket(family, SocketType.Stream, ProtocolType.Tcp);
                    await clients[i].ConnectAsync(listener.LocalEndPoint!).WaitAsync(TestSettings.PassingTestTimeout);
                }
                for (int i = 0; i < clients.Length; i++)
                {
                    Assert.True(await (i == 0 ? first : enumerator.MoveNextAsync().AsTask()).WaitAsync(TestSettings.PassingTestTimeout));
                    using Socket accepted = enumerator.Current;
                    Assert.True(accepted.Connected);
                    Assert.Equal(clients[i].LocalEndPoint, accepted.RemoteEndPoint);
                    Assert.Equal(clients[i].RemoteEndPoint, accepted.LocalEndPoint);
                    Assert.Equal(listener.Blocking, accepted.Blocking);
                    Assert.Equal(family, accepted.AddressFamily);
                }
            }
            finally
            {
        ...
        public async Task Cancellation_LeavesListenerUsable(bool enumeratorToken, bool precanceled)
        {
            ...
            await using (IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync(
        ...
        [Fact]
        public async Task ListenerDisposal_CompletesPendingAccept()
        {
            using Socket listener = CreateListener();
            await using IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> pending = enumerator.MoveNextAsync().AsTask();
            Assert.False(pending.IsCompleted);
            await Task.Run(listener.Dispose).WaitAsync(TestSettings.PassingTestTimeout);
        ...
        [Fact]
        public async Task DisposeEnumerator_ClosesUnclaimedSocketsOnly()
        {
            using Socket listener = CreateListener();
            IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> first = enumerator.MoveNextAsync().AsTask();
            Socket[] clients = new Socket[32];
            try
            {
                for (int i = 0; i < clients.Length; i++)
                {
                ...
                Assert.True(await first.WaitAsync(TestSettings.PassingTestTimeout));
                using Socket accepted = enumerator.Current;
                await enumerator.DisposeAsync().AsTask().WaitAsync(TestSettings.PassingTestTimeout);
                listener.Dispose();

                byte[] data = { 42 };
                Assert.Equal(1, clients[0].Send(data));
                byte[] received = new byte[1];
                Assert.Equal(1, await accepted.ReceiveAsync(received.AsMemory()).AsTask().WaitAsync(TestSettings.PassingTestTimeout));
                ...
                for (int i = 1; i < clients.Length; i++)
                {
                    try
                    {
                        Assert.Equal(0, await clients[i].ReceiveAsync(received.AsMemory()).AsTask().WaitAsync(TestSettings.PassingTestTimeout));
                    }
        ...
        [Fact]
        public async Task ConcurrentEnumerators_HaveIndependentCancellation()
        {
            using Socket listener = CreateListener();
            using CancellationTokenSource cancellation = new();
            await using IAsyncEnumerator<Socket> first = listener.AcceptMultishotAsync(cancellation.Token).GetAsyncEnumerator();
            await using IAsyncEnumerator<Socket> second = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> firstAccept = first.MoveNextAsync().AsTask();
            Task<bool> secondAccept = second.MoveNextAsync().AsTask();
            cancellation.Cancel();
            await Assert.ThrowsAnyAsync<OperationCanceledException>(() => firstAccept.WaitAsync(TestSettings.PassingTestTimeout));
            using Socket client = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            await client.ConnectAsync(listener.LocalEndPoint!);
            Assert.True(await secondAccept.WaitAsync(TestSettings.PassingTestTimeout));
            using Socket accepted = second.Current;
            Assert.Equal(client.LocalEndPoint, accepted.RemoteEndPoint);
        }

        [Fact]
        public async Task ResetAfterYield_PreservesRemoteEndpoint()
        {
            using Socket listener = CreateListener();
            await using IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> pending = enumerator.MoveNextAsync().AsTask();
            using Socket client = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            await client.ConnectAsync(listener.LocalEndPoint!);
            EndPoint remoteEndpoint = client.LocalEndPoint!;
            Assert.True(await pending.WaitAsync(TestSettings.PassingTestTimeout));
            using Socket accepted = enumerator.Current;
            client.LingerState = new LingerOption(true, 0);
            client.Dispose();
            SocketException exception = await Assert.ThrowsAsync<SocketException>(() =>
                accepted.ReceiveAsync(new byte[1].AsMemory()).AsTask().WaitAsync(TestSettings.PassingTestTimeout));
            Assert.Equal(SocketError.ConnectionReset, exception.SocketErrorCode);
            Assert.Equal(remoteEndpoint, accepted.RemoteEndPoint);
        }

        [Fact]
        public async Task ResetPrefetchedConnection_DoesNotEndSequence()
        {
            using Socket listener = CreateListener();
            await using IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> first = enumerator.MoveNextAsync().AsTask();
            using Socket initialClient = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            await initialClient.ConnectAsync(listener.LocalEndPoint!);
            Assert.True(await first.WaitAsync(TestSettings.PassingTestTimeout));
            using Socket initialAccepted = enumerator.Current;

            using Socket resetClient = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            await resetClient.ConnectAsync(listener.LocalEndPoint!);
            resetClient.LingerState = new LingerOption(true, 0);
            resetClient.Dispose();
            using Socket liveClient = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            await liveClient.ConnectAsync(listener.LocalEndPoint!);
            for (int i = 0; i < 2; i++)
            {
                Assert.True(await enumerator.MoveNextAsync().AsTask().WaitAsync(TestSettings.PassingTestTimeout));
                using Socket accepted = enumerator.Current;
                if (Equals(liveClient.LocalEndPoint, accepted.RemoteEndPoint))
                {
                    return;
                }
            }
            Assert.Fail("The live connection was not accepted after a peer reset.");
        }

        [Fact]
        public async Task NonOwningListenerDisposal_DoesNotCloseDescriptor()
        {
            using Socket owner = CreateListener();
            using Socket listener = new(new SafeSocketHandle(owner.SafeHandle.DangerousGetHandle(), ownsHandle: false));
            await using IAsyncEnumerator<Socket> enumerator = listener.AcceptMultishotAsync().GetAsyncEnumerator();
            Task<bool> pending = enumerator.MoveNextAsync().AsTask();
            await Task.Run(listener.Dispose).WaitAsync(TestSettings.PassingTestTimeout);
            SocketException exception = await Assert.ThrowsAsync<SocketException>(() => pending.WaitAsync(TestSettings.PassingTestTimeout));
            Assert.Equal(SocketError.OperationAborted, exception.SocketErrorCode);
            using Socket client = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            await client.ConnectAsync(owner.LocalEndPoint!);
            using Socket accepted = owner.Accept();
            Assert.Equal(client.LocalEndPoint, accepted.RemoteEndPoint);
        }

        [Fact]
        public void InvalidListener_Throws()
        {
            using Socket socket = new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            Assert.Throws<InvalidOperationException>(() => socket.AcceptMultishotAsync());
            socket.Bind(new IPEndPoint(IPAddress.Loopback, 0));
            Assert.Throws<InvalidOperationException>(() => socket.AcceptMultishotAsync());
            socket.Dispose();
            Assert.Throws<ObjectDisposedException>(() => socket.AcceptMultishotAsync());
        }
    }
