...
{
    // EXPERIMENTAL, PROTOTYPE-ONLY: implements "Architecture B" from the io_uring design doc for
    // TCP/stream sockets - true completion-based Receive/Send/Accept/Connect submitted directly via
    // io_uring, using the public System.Threading.IoUring API to reuse the same per-thread ring a
    // Thread Pool worker already owns for file I/O (see System.Private.CoreLib's
    // PortableThreadPool.IoUring.Unix.cs). This is purely additive: every method here either
    // completes the operation by invoking the caller's callback asynchronously (returning true), or
    // returns false immediately without side effects, in which case the caller falls back to the
    // existing epoll-based SocketAsyncEngine path unchanged. UDP/datagram sockets, multi-buffer
    // scatter/gather, and cancellation are all out of scope - callers only attempt these methods for
    // the plain single-buffer / no-destination-address cases; anything else always returns false.
