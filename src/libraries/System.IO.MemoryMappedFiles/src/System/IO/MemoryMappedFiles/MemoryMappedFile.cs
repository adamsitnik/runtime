...
namespace System.IO.MemoryMappedFiles
{
    public partial class MemoryMappedFile : IDisposable
    {
        ...
        private MemoryMappedFile(SafeMemoryMappedFileHandle handle, SafeFileHandle safeFileHandle, bool leaveOpen)
        {
            Debug.Assert(handle != null);
            Debug.Assert(!handle.IsClosed);
            Debug.Assert(!handle.IsInvalid);
            Debug.Assert(safeFileHandle != null);
        ...
        // the MemoryMappedFile will close the SafeFileHandle object when it is disposed.  This behavior can be
                                                                        ...
                                                                        MemoryMappedFileAccess access)
        {
            ...
            SafeFileHandle safeFileHandle = File.OpenHandle(path, mode, GetFileAccess(access), FileShare.Read, FileOptions.None);

            long fileSize = mode == FileMode.Create || mode == FileMode.CreateNew
                ? 0 // we've just created the file, so we know it's empty
                : RandomAccess.GetLength(safeFileHandle);
            ...
            try
            {
                handle = CreateCore(safeFileHandle, mapName, HandleInheritability.None,
            ...
            Debug.Assert(handle != null);
            Debug.Assert(!handle.IsInvalid);
            return new MemoryMappedFile(handle, safeFileHandle, false);
        }
                                                        ...
                                                        HandleInheritability inheritability, bool leaveOpen)
        {
            ...
            SafeFileHandle safeFileHandle = fileStream.SafeFileHandle; // access the property only once, as it performs a sys-call (Seek)
            SafeMemoryMappedFileHandle handle = CreateCore(safeFileHandle, mapName, inheritability,
                access, MemoryMappedFileOptions.None, capacity, fileSize);
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            {
                if (_safeFileHandle != null && _leaveOpen == false)
                {
        ...
        private static void CleanupFile(SafeFileHandle safeFileHandle, bool existed, string path)
        {
